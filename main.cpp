...
void loadData(int inputs){
	...
	ifstream data_inputs2("../data/test/inRNN_11.txt"); //opening an input stream for file test.txt
	ifstream data_outputs2("../data/test/outRNN.txt"); //opening an input stream for file test.txt
	long test_counter = 0;
...
for (int q=0;q<20;q++){
	...
	for (int a=0;a<100;a++){
		...
		for (int i=0;i<mini_batch;i++){
			...
			std::random_device rd; // obtain a random number from hardware
	   	 	std::mt19937 eng(rd()); // seed the generator
			std::uniform_real_distribution<> distr(0, 33475); // define the range
		...
		cout<<(double)errors/55007<<endl;
		saveFile1 << (a+1)*10<< " " <<(double)errors/55007<<"\n";
		//cout<<ann.dumpWeights()<<endl;
	}
}
//	
